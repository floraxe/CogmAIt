...
def verify_password(plain_password: str, hashed_password: str) -> bool:
    """验证密码是否匹配（与get_password_hash配套，先SHA256压缩再验证）"""
    # 必须与get_password_hash做相同的SHA256压缩，否则无法匹配
...
def get_password_hash(password: str) -> str:
    """
    生成密码哈希（彻底解决bcrypt 72字节限制）
    先通过SHA256将任意长度密码压缩为固定64字符，再进行bcrypt哈希
    """
    # 万能修复：不管密码多长，先SHA256压缩为固定64字符（远小于72字节）
